...
/**
 * 32-byte key from SENTI_ENCRYPTION_KEY. Without it we refuse to encrypt
 * rather than silently storing biometrics in the clear.
 */
...
/** Encrypt a voiceprint payload. Returns ciphertext, or the plaintext if no key is set. */
export function encryptSecret(plaintext: string): string {
  const key = encryptionKey()
  if (!key) return plaintext // no key configured — caller warns
